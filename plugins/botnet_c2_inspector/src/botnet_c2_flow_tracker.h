#ifndef BOTNET_C2_FLOW_TRACKER_H
#define BOTNET_C2_FLOW_TRACKER_H
...
static constexpr unsigned AGG_FEATURE_COUNT = 8;

struct BotC2ScalerParams {
...
struct BotC2Profile {
    ...
    double latest_i_mean;
    double latest_i_std;

    bool inference_done;

    void reset(uint32_t ip, double ts) {
        dst_ip = ip; window_start_ts = ts;
        syn_count = 0; syn_src_ips.clear();
        syn_dst_ports.clear(); syn_dst_port_counts.clear(); syn_src_ports.clear();
        syn_timestamps.clear();
        latest_i_mean = 0; latest_i_std = 0;
        inference_done = false;
    }

    void add_syn(uint32_t sip, uint16_t dport, uint16_t sport, double ts) {
        syn_count++;
        syn_src_ips.insert(sip);
        syn_dst_ports.insert(dport);
        syn_dst_port_counts[dport]++;
        syn_src_ports.insert(sport);
    ...
    void compute_features(double* raw, double window_sec) const {
        ...
        raw[2] = iat_cv();
        raw[3] = static_cast<double>(syn_dst_ports.size());
        raw[4] = static_cast<double>(syn_src_ports.size());
    ...
    static void preprocess(double* f, const BotC2ScalerParams& p) {
