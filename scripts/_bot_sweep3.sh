#!/bin/bash
# Threshold sweep with fixed binary (private IP filter applied)
...
echo "=== Bot Client Threshold Sweep v2 (private IP filter active) ==="
echo ""

for THR in 0.85 0.75 0.60 0.50 0.40 0.30; do
    LOGDIR="/tmp/bot_v2_${THR//./}"
    mkdir -p "$LOGDIR"
    rm -f "$LOGDIR/alert_csv.txt"

    CFG_TMP="/tmp/snort_botv2_${THR//./}.lua"
    cat > "$CFG_TMP" << LUAEOF
HOME_NET     = 'any'
EXTERNAL_NET = 'any'
...
LUAEOF
    ...
    # Parse src IPs from alert_csv: field 7 = src:port
