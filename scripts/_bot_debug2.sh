...
mkdir -p /tmp/bot_debug2
rm -f /tmp/bot_debug2/alert_csv.txt

echo "=== Bot debug with private IP filter: threshold=0.10 ==="
cd "$SNORT_ETC" && snort -c "$CFG" --plugin-path "$PLUGIN_PATH" \
    -r "$HOME/bitirme/pcaps/Friday-WorkingHours.pcap" \
    -A alert_csv -l /tmp/bot_debug2 -q 2>&1 | \
    grep '\[botcl\]' | head -20

echo "=== All unique alerted src IPs ==="
awk -F', ' '{split($7,a,":"); print a[1]}' /tmp/bot_debug2/alert_csv.txt 2>/dev/null | sort -u
