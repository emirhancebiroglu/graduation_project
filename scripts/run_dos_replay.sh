#!/bin/bash
# run_dos_replay.sh — CIC-IDS2017 PCAP'larını DoS Inspector ile çalıştır
# Kullanım: cd ~/bitirme && bash scripts/run_dos_replay.sh
...
CONFIG="$HOME/bitirme/configs/snort_dos.lua"
PLUGIN_PATH="$HOME/bitirme/plugins/dos_inspector/build"
PCAP_DIR="$HOME/bitirme/pcaps"
OUTPUT_DIR="$HOME/bitirme/results/dos_inspector"
...
echo "============================================="
echo " DoS Inspector — CIC-IDS2017 PCAP Replay"
echo "============================================="
echo "Config:      $CONFIG"
...
# Plugin kontrolü
if [ ! -f "$PLUGIN_PATH/dos_inspector.so" ]; then
    echo "HATA: dos_inspector.so bulunamadı!"
    echo "Önce derleme yapın: cd ~/bitirme/plugins/dos_inspector && ./build.sh"
    exit 1
fi
...
echo "  python scripts/dos_flowid_confusion.py \\"
echo "      --alert-dir ~/bitirme/results/dos_inspector \\"
echo "      --csv-dir ~/bitirme/data/raw/cicids2017 \\"
echo "      --output ~/bitirme/results/dos_inspector/confusion_matrix.txt"
