#!/usr/bin/env python3
"""
lstm_flowid_confusion_wednesday.py — LSTM Inspector confusion matrix (Wednesday only)
...
Kullanım:
    python lstm_flowid_confusion_wednesday.py \
        --alert-dir ~/bitirme/results/lstm \
        --csv-dir ~/bitirme/data/raw/cicids2017 \
        --output ~/bitirme/results/lstm/confusion_matrix_wednesday.txt
"""
...
def extract_flow_ids_from_alerts(alert_dir: Path) -> set:
    ...
    for subdir in sorted(alert_dir.iterdir()):
        ...
        logging.info(f"  Karşılaştırma: {repr(subdir.name.lower())} == {repr(WEDNESDAY_ALERT_SUBDIR.lower())} → {subdir.name.lower() == WEDNESDAY_ALERT_SUBDIR.lower()}")
        if subdir.name.lower() != WEDNESDAY_ALERT_SUBDIR.lower():
...
def main():
    ...
    logging.info("=" * 60)
    logging.info("LSTM Alert → Flow ID (Wednesday-only, IP haritalama ile)")
    logging.info("=" * 60)
    ...
    result = f"""
{'=' * 60}
LSTM Inspector — CIC-IDS2017 Wednesday Confusion Matrix
