#!/usr/bin/env python3
"""
xgb_flowid_confusion.py — XGBoost Inspector confusion matrix

lstm_flowid_confusion.py ile aynı mantık.
Fark: --alert-dir varsayılanı xgboost dizinine işaret eder.

Kullanım:
    python xgb_flowid_confusion.py \
        --alert-dir ~/bitirme/results/xgboost \
        --csv-dir ~/bitirme/data/raw/cicids2017 \
        --output ~/bitirme/results/xgboost/confusion_matrix.txt
"""
...
def main():
    ...
    logging.info("=" * 60)
    logging.info("XGBoost Alert → Flow ID (IP haritalama ile)")
    logging.info("=" * 60)
    ...
    result = f"""
{'=' * 60}
XGBoost Inspector — CIC-IDS2017 Confusion Matrix
