...
ALERT_DIR=$HOME/bitirme/results/portscan/Wednesday-WorkingHours
mkdir -p "$ALERT_DIR"
...
echo "Exit code: $?"
echo "Cross-flow alerts on Wednesday:"
grep -c 'portscan.*ALERT' "$ALERT_DIR/snort_output.log" 2>/dev/null || echo 0
echo "---"
grep 'portscan.*ALERT' "$ALERT_DIR/snort_output.log" | head -10
